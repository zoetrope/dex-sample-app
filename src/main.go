...
func (a *app) handleCallback(w http.ResponseWriter, r *http.Request) {
	...
	session.Values["claims"] = []byte(claims)
...
func (a *app) handleTest(w http.ResponseWriter, r *http.Request) {
	...
	rawIDToken := session.Values["id_token"].(string)
	accessToken := session.Values["access_token"].(string)
	refreshToken := session.Values["refresh_token"].(string)
	claims := session.Values["claims"].([]byte)
	...
	renderToken(w, a.redirectURI, rawIDToken, accessToken, refreshToken, buff.String())
}
